...
%   17 Mar 2017 (CT)   Added Anthocyanins (following Prospect-D)
%
% usage:
...
%     [leafopt.MbI_rc,leafopt.MfI_rc] = deal(0.5*fqe(1) * ((phiI(Iwlf)))  * kChlrel(Iwle)'.*sigmoid); % fluxes without reabsorption
%     [leafopt.MbII_rc,leafopt.MfII_rc] = deal(0.5*fqe(2) * ((phiII(Iwlf))) * kChlrel(Iwle)'.*sigmoid);% fluxes without reabsorption
