...
%% 3. file names
[dummy,X]                       = xlsread(['../' char(parameter_file)],'filenames'); %#ok<ASGLU>
j = ~strcmp(X(:,2),{''});
X = X(j,(1:end));
...
if options.simulation == 1
    vi = ones(length(V),1);
    [soil,leafbio,canopy,meteo,angles,xyt]  = select_input(V,vi,canopy,options); %#ok<ASGLU>
...
[rad,thermal,fluxes] = initialize_output_structures(spectral); %#ok<ASGLU>
        ...
        switch options.calc_ebal
            ...
            otherwise
                ...
                if options.calc_fluor
                    ...
                    if options.calc_vert_profiles
                        [rad,profiles] = RTMf(spectral,rad,soil,leafopt,canopy,gap,angles,profiles); %#ok<NASGU>
                    else
                        [rad] = RTMf(spectral,rad,soil,leafopt,canopy,gap,angles,profiles);
                    end
                end
