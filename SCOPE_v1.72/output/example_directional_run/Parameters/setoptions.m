...
1;		%calc_planck		calculate spectrum of thermal radiation with spectral emissivity instead of broadband
1;		%calc_directional		calculate BRDF and directional temperature for many angles specified in a file. Be patient, this takes some time
...
2;		%SoilHeatMethod		0: standard calculation of thermal inertia from soil characteristics, 1: empiricaly calibrated formula (make function), 2: as constant fraction of soil net radiation
...
0];		%simulation		0: individual runs. Specify one value for constant input, and an equal number (>1) of values for all input that varies between the runs.
