...
B   = soilpar.BSMBrightness;        % soil brightness (range = 0 - 0.9)
...
function rwet = soilwat(rdry,nw,kw,SMp,SMC,deleff)
    ...
    rbac = 1 - (1-rdry) .* (rdry .* equations.tav(90,2.0./nw) / equations.tav(90,2.0) + 1-rdry);
    
    % total reflectance at bottom of water film surface
    
    p    = 1 - equations.tav(90,nw) ./ nw.^2;
    
    % reflectance of water film top surface, use 40 degrees incidence angle, 
    % like in PROSPECT
    
    Rw  = 1 - equations.tav(40,nw);
