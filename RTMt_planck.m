...
for i = 1:length(IT);
    ...
    Hcsui3          = Planck(wlt(i),Tcu+273.15,epsc(i));
    Hcshi           = Planck(wlt(i),Tch+273.15,epsc(i));
    Hssui           = Planck(wlt(i),Tsu+273.15,epss(i));
    Hsshi           = Planck(wlt(i),Tsh+273.15,epss(i));
    ...
    if obsdir
        ...
        piLo1           = iLAI*epsc(i)*K*sum(Hcshi.*(Po(1:nl)-Pso(1:nl)));         % directional   emitted     radiation by shaded leaves        
        piLo2           = iLAI*epsc(i)  *sum(Hcsui.*(Pso(1:nl         )));         % directional   emitted     radiation by sunlit leaves
        piLo3           = iLAI*((vb(i)*Emini(1:nl) + vf(i)*Eplui(1:nl))'*Po(1:nl));% directional   scattered   radiation by vegetation for diffuse incidence 
        piLo4           = epss(i)*Hsshi*(Po(nl+1)-Pso(nl+1));
        piLo5           = epss(i)*Hssui* Pso(nl+1);                                % directional   emitted     radiation by sunlit/shaded Soil
