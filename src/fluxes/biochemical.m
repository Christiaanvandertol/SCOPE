function biochem_out = biochemical(leafbio,meteo,options,~,fV,Q)
%
% Date: 	21 Sep 2012
% Update:   20 Feb 2013
...
%model_choice  = leafbio.Fluorescence_model;
if isfield(leafbio, 'useTLforC3')
    useTLforC3   = leafbio.useTLforC3;
else
    useTLforC3 = false;
end
    
  % fluoeresence
if isfield(leafbio, 'Knparams')
    Knparams      = leafbio.Knparams;
elseif isfield( leafbio, 'Kn0')
    Knparams = [leafbio.Kn0, leafbio.Kn_alpha, leafbio.Kn_beta];
elseif isfield(options, 'Fluorescence_model') && options.Fluorescence_model==0
...
if isfield(leafbio, 'Tparam')
   Tparams = leafbio.Tparam;
    slti        = Tparams(1);
    shti        = Tparams(2);
    Thl         = Tparams(3);
    Thh         = Tparams(4);
