...
fidp            = fopen([Output_dir,'BOC_irradiance.dat'],'w');
...
if options.calc_fluor
    ...
    fidfle         = fopen([Output_dir,'fluorescence_emitted_by_all_leaves.dat'],'w');
    fidfrc         = fopen([Output_dir,'fluorescence_emitted_by_all_photosystems.dat'],'w');
    fidflsu      = fopen([Output_dir,'fluorescence_sunlit.dat'],'w');     % Fluorescence
    fidflsh      = fopen([Output_dir,'fluorescence_shaded.dat'],'w');     % Fluorescence
    fidflsc      = fopen([Output_dir,'fluorescence_scattered.dat'],'w');     % Fluorescence
else
...
if options.save_headers
    ...
    fprintf(fidp, 'Bottom of canopy irradiance in the shaded fraction, and average BOC irradiance \r');
    fprintf(fidp, 'First 2162 columns: shaded fraction. Last 2162 columns: average BOC irradiance. Unit: Wm-2 um-1 \r');
    
   % fprintf(fidref2, 'reflectance including dynamic Xanthophyll effects \r');
   % fprintf(fidref2, 'fraction of radiation in observation direction *pi / irradiance \r');
    ...
    if options.calc_fluor
        ...
        fprintf(fidfle, 'total emitted fluorescence by all leaves for wavelengths of 640 to 850 nm, with 1 nm resolution \r');
        fprintf(fidfle, 'W m-2 um-1 \r');
        fprintf(fidfrc, 'total emitted fluorescence by all photosystems for wavelengths of 640 to 850 nm, with 1 nm resolution \r');
        fprintf(fidfrc, 'W m-2 um-1 \r');
        fprintf(fidflsu, 'TOC fluorescence contribution from sunlit leaves for wavelengths of 640 to 850 nm, with 1 nm resolution \r');
        fprintf(fidflsu, 'W m-2 um-1 sr^{-1} \r');
        fprintf(fidflsh, 'TOC fluorescence contribution from shaded leaves for wavelengths of 640 to 850 nm, with 1 nm resolution \r');
        fprintf(fidflsh, 'W m-2 um-1 sr^{-1} \r');
        fprintf(fidflsc, 'TOC fluorescence contribution from from leaves and soil after scattering for wavelenghts of 640 to 850 nm, with 1 nm resolution \r');
        fprintf(fidflsc, 'W m-2 um-1 sr^{-1} \r');
