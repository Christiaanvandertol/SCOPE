...
The data read from files in *"../data/input/dataset X"* will be subset to your startDOY, endDOY.

SCOPE will make runs for every time stamp in your data.

It is possible to interpolate variable that are measured less frequently than meteorological data (:ref:`directories/data:vegetation_retrieved_csv`).
...
.. figure:: ../images/ts.png
    ...
    Results of time series run from "../data/input/:ref:`directories/data:dataset for_verification`".
    Each point - single SCOPE run.
    Left column - inputs, right - outputs from :ref:`outfiles/each:fluxes.dat`.
    Blue - standard 'verificationdata' dataset, red - 'verificationdata_vcmax_cab', where Vcmax and Cab values are interpolated.
